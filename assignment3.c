...
int count_occurrences_of_substr(const char *data, const char *s_p) {
    ...
    for (int i = 0; i <= n - m; i++) {
        ...
        // If we matched the full substring, increment the count
        if (j == m) {
            full_count++;
        }
    }
...
void *handle_client(void *client_socket) {
    ...
    while (1) {
        ...
        if (bytes_read > 0) {
            ...
            pthread_mutex_lock(&linked_list_mutex);
            if (!analysis_flag) {  // Only trigger analysis if it's not already running
...
int main(int argc, char *argv[]) {
    if (argc < 5) {
        fprintf(stderr, "Usage: %s -l <port> -p <search_pattern>\n", argv[0]);
        exit(EXIT_FAILURE);
    }
